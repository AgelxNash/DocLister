    ...
    Plugin.prototype = {
        ...
        _getFiles: function (evt, fn) {
            var
                ...
                , data = {
                    all: files, files: [], other: [], duplicate: opts.duplicate ? [] : this._extractDuplicateFiles(files)
                }
        ...
        _getUploadEvent: function (extra) {
            var xhr = this.xhr, evt = {
                xhr: xhr, file: xhr.currentFile, files: xhr.files, widget: this
            };
        ...
        _emitCompleteEvent: function (prefix, err, xhr, file) {
            var evt = this._getUploadEvent({
                error: err, status: xhr.status, statusText: xhr.statusText, result: xhr.responseText
            });
            ...
            _each(files, function (file, i) {
                ...
                if ($files.length && !this.$file(uid).length) {
                    var html = this.itemTplFn({
                        $idx: offset + i, uid: uid, name: file.name, type: file.type, size: file.size, sizeText: this._getFormatedSize(file.size)
                    });
        ...
        upload: function () {
            if (!this.active) {
                ...
                var
                    ...
                    , files = {}
                    , uploadOpts = {
                        url: opts.url, data: _extend({}, this.serialize(), opts.data), headers: opts.headers, files: files, chunkSize: opts.chunkSize | 0, chunkUploadRetry: opts.chunkUploadRetry | 0, prepare: _bind(this, this._onFileUploadPrepare), imageTransform: opts.imageTransform
                    }
                    ;
            ...
            if ($el.length) {
                api.getInfo(file, _bind(this, function (err, info) {
                    if (err) {
                        ...
                        $el.find('>div>div').css({
                            width: Math.round(rx * info[coords.flip ? 'height' : 'width']), height: Math.round(ry * info[coords.flip ? 'width' : 'height']), marginLeft: -Math.round(rx * coords.rx), marginTop: -Math.round(ry * coords.ry)
                        });

                        if (preview.keepAspectRatio) { // create side gaps
                            $el.find('>div').css({
                                width: Math.round(mx), height: Math.round(my), marginLeft: mx < pw ? Math.round((pw - mx) / 2) : 0, marginTop: my < ph ? Math.round((ph - my) / 2) : 0
                            });
                        }
                    }
                }));
        ...
        rotate: function (file, deg) {
            ...
            $el.css({
                '-webkit-transform': 'rotate(' + deg + 'deg)', '-moz-transform': 'rotate(' + deg + 'deg)', 'transform': 'rotate(' + deg + 'deg)'
            });
        },
                    ...
                    _each(['onSelect', 'onChange'], function (name, fn) {
                        if (fn = opts[name]) {
                            opts[name] = function (coords) {
                                ...
                                fn({
                                    x: x, y: y, w: w, h: h, rx: fx * (flip ? oh : ow), ry: fy * (flip ? ow : oh), rw: fw * (flip ? oh : ow), rh: fh * (flip ? ow : oh), lx: coords.x // local coords
                                    , ly: coords.y, lw: coords.w, lh: coords.h, deg: deg, flip: flip
                                });
                            };
                        }
                    });
