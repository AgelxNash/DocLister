...
class tv_DL_filter extends content_DL_filter{
	...
	function parseFilter($filter) {
		...
		$tvid = $this->modx->db->select('id', $this->DocLister->getTable('site_tmplvars'), 'name = "' . $this->field . '"');
		$tvid = $this->modx->db->makeArray($tvid);
		$this->tv_id = intval($tvid[0]['id']);
		if (!$this->tv_id) $this->modx->logEvent(0, 2, 'DocLister filtering by template variable "' . $this->field . '" failed. TV not found!');
	...
	function get_join(){
		$join = 'LEFT JOIN ' . $this->DocLister->getTable('site_tmplvar_contentvalues') . ' ' . $this->tv_table_alias
